...
if __name__ == "__main__":
    ...
    parser.add_argument('paths', nargs='*', type=str, metavar='p', help='Put input path first, then output path,'
    ...
    if args.prev_sky:
        gen_prev_sky_image(science_data_directory=args.paths[0],output_directory=args.paths[1],previous_sky=args.paths[2],
                      sky_group_size=sky_size,sigma=args.sigma[0])
    else:
        gen_sky_image(science_data_directory=args.paths[0], output_directory=args.paths[1], sky_group_size=sky_size,sigma=args.sigma[0])
