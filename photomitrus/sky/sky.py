...
def subtract_sky_and_normalize(science_data_directory, output_data_dir, sky):
    ...
    image_fnames = [os.path.join(science_data_directory, f) for f in os.listdir(science_data_directory) if f.endswith('flat.fits') or f.endswith('ramp.new')]
    image_fnames.sort()
    sky = fits.getdata(sky)
    cropsky = sky#[4:4092, 10:]  #remove if crop issue ever fixed
    print('Cropping and sky subtracting input images...')
