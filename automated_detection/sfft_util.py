...
def get_good_mask(cat):
	...
	flux_rad_col_no_noise = flux_rad_col[flux_rad_col>px_per_arcsec] # larger than an arcsec
	
	med, std = np.median(flux_rad_col_no_noise), np.std(flux_rad_col_no_noise)

	mask = (cat['FLAGS'] == 0)  & (flux_rad_col > med - std) & (cat['ELONGATION'] < 2) 
	return mask
...
def ztf_cuts(cat, PixA_DIFF, crop=300):
	...
	"""
	Apply source cuts to difference image to retain only good sources
	"""
...
def closest_source(sources, FITS_DIFF, ra, dec):
	...
	sources = ztf_cuts(sources, FITS_DIFF) # reshaping 
...
def SFFT(FITS_SCI, FITS_REF):
	...
	# savename, threshname, source_ra, source_dec = closest_source(sources, FITS_DIFF, ra, dec)
	
	return FITS_SCI, FITS_REF, FITS_DIFF, PixA_DIFF, SFFTPrepDict
