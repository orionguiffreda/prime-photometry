...
def sex1(imageName, det_cut, weightName=None, magtype='PSF'):
	"""
	Run source extractor with aggressive config
	"""
...
def get_reg(imageName, reg_name = "GRB_query_thresh.reg"):
	"""
	Read in relevant region file(s) for source cutout
	"""
	dir_name = os.path.dirname(imageName)
	reg_files = glob.glob(f"{dir_name}/{reg_name}")
	plt_primeregs_all = []
	print(reg_files)
